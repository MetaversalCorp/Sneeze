...
namespace SNEEZE
{
   ...
   public:
     ...
     ~CONTAINER ();
      ...
      // True when the node was opened by the map service (present in its registry).
      // WASM-injected and static-MSF nodes return false and are thus exempt from
      // proximity streaming. False when the container has no map service.
