...
{
public:
   ILOGGERApp (SNEEZE::ENGINE* pEngine) :
      m_pEngine (pEngine)
   {
   }
   ...
   void onMessage (uint64_t tNow, RMAP::CORE::LOGGER::eLOGLEVEL eLevel, std::string& sModule, std::string& sLine) override
   {
      std::string s = "RMAP (" + sModule + ")";
      m_pEngine->Log ((SNEEZE::IENGINE::eLOGLEVEL)eLevel, s, sLine);
   }
...
public:
   ...
   bool Initialize (const std::string& sPath_Root)
   {
      ...
      m_pLoggerApp = new ILOGGERApp (m_pEngine);

      pCore->LoggerStart (m_pLoggerApp, true);
