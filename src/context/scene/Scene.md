...
| `Animation_Tick()` | Loop clip 0 of the attached model, or a retargeted VRMA clip when `Resource.aSupplementary` `"vrma"` is loaded, into the node's palettes using a persistent node-tree workspace. No-op when the model has no clip or no skins. |
...
- **glTF/GLB render model** — `NODE::Gltf_Render_Model(GLTF_RENDER_MODEL*)`
  ...
  buffers. `NODE::Animation_Tick` (called by the compositor before emit) loops
