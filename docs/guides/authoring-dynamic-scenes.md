...
pub extern "C" fn Open(twFabricIx: u64, _dwOffset: u32, _dwLength: u32)
{
   ...
   box_node.d3Position        = [-3.0, 0.0, 0.0];
...
The same module in `tests/wasm/solar_panel` adds one more thing: an in-scene **UI panel** built with `Node_Panel`, a call that has no JSON equivalent. It hands the engine an RML+CSS document (RmlUi's HTML-like markup) that the engine rasterizes to a textured, camera-facing quad. That is the only way to put a UI surface in a scene, and it is covered on the host-API page.
