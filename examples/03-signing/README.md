...
The scene is identical to Example 02, so nothing in the `data` tree is new. The two new things are a `hash` on the module, and the signing step that turns the plain `signed-stool-and-bucket.json` into the signed `signed-stool-and-bucket.msf`. Everything about the scene itself - `container`, the `Children` tree, `Transform`, and the lights - works exactly as [Example 02](../02-stool-and-bucket/README.md) explained.
