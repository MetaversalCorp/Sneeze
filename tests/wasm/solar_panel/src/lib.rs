...
{
   LogMsg ("Solar Panel WASM: Init");
}

#[no_mangle]
pub extern "C" fn Open (twFabricIx: u64, _dwOffset: u32, _dwLength: u32)
{
   LogMsg (&format! ("Solar Panel WASM: Open (twFabricIx={})", twFabricIx));

   // The "Solar Panel" frame (index 2, STARSYSTEM) is the fabric's root node.
   ...
   // textured quad. Demonstrates Node_Panel + UI_PANEL::Source. font-family
   // "Inter" matches the fonts the engine's UI context loads.
   const PANEL_RML: &str =
      "<rml><head><style>\
       ...
       <body><div id='card'>\
       <span class='title'>Solar Panel</span>\
...
{
   LogMsg ("Solar Panel WASM: Close");
}

#[no_mangle]
pub extern "C" fn Shutdown ()
{
   LogMsg ("Solar Panel WASM: Shutdown");
}
