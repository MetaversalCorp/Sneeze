...
## July 31, 2026 (~1:30 PM - 2:35 PM PT) - Dean Abramson

**Reviewed/tidied the date-time-timer work and prepped the five-repo release. (Full detail in `project.mdc` "Date, Time & Timers" section.)**

- **Timer code review.** Simplified four element-access loops in `Wasm_Timers.cpp` to bind `ENTRY& Entry = m_aEntry[i]` once (Dean's `m_aEntry + i` first pass didn't compile - it's a `std::vector`; corrected to a reference). Walked through references vs pointers (the `&`-in-declaration = alias).
- **Deferred convention:** adopt an `r` prefix for references (like `p` for pointers) - all-or-nothing, bundled with the `qwComposed` sweep. Recorded in `e:\dev\FIX-OBJECT-VARIABLE-NAMES.mdc` §11.
- **Concurrency walk-through:** per-store `m_mutex` serializes all host->guest entries (timers mutually exclusive with Instance_Open/Close); `Notify_Guest` trusts the caller to hold the lock (future callbacks must too); non-recursive lock => no inline re-entry; the `AgentTimer` null checks are defensive-only (runtime outlives the TIMER agents by construction).
- **KNOWN ISSUE logged (perf only, deferred):** TIMER-pool head-of-line blocking - a hot store can park multiple agents (~25%/agent, up to ~75%). Fix = per-store single-flight in `Claim`. Noted in `Wasm.md` + a `TODO(perf)` on `WASM_TIMERS::Claim`.
- **Gitignore:** replaced stale `tests/wasm/*/target/` with general `**/wasm/**/target/` (crates now under `tools/wasm/*`); verified via `git check-ignore`.
- **Deleted the `sdk/` junction** (`sdk/rust` -> `deps/repos/SneezeSDK/Rust`) safely via `cmd /c rmdir`. Consequence: `testapi`'s committed `Cargo.toml` path dangles until repointed. Parked: the SDK local-dev workflow (options captured in `project.mdc`).
- **Docs + resume point:** confirmed `Wasm.md` (+97), `Control.md` (+19), and the SneezeSDK api pages are current; wrote the full "Date, Time & Timers" resume-point section in `project.mdc`.
- **Release in progress:** Dean delegated the git work to the AI this time (one-off exception to his always-commits-himself rule). Order: 3 SDK submodules -> SneezeSDK superproject -> Sneeze.
- Cursor UTF-16 write bug hit `Wasm_Timers.cpp` on one edit; re-encoded. Keep byte-verifying.

## August 3, 2026 (~6:45 PM - 8:00 PM PT) - Dean Abramson

**Post-`-Sync` audit: found + fixed two real bugs in the dependency-sync tooling, then brought the README fully current. (Full detail in `project.mdc` dependency single-source-of-truth section.)**

- **halogen was missing from the `-Sync` rebuild list (22 of 23).** Root cause: anari-sdk moved in Dean's crashed first `-Sync` (boringssl SHA-as-tag error), so the resume run saw it already at the pin (not "moved this run") and halogen's surviving `.done` stamp hid it. Fix: `-Sync` now cascades rebuilds to dependents of the WHOLE rebuild set (moved + missing-stamp), not just what moved — both scripts, docs updated. Dean force-rebuilt halogen once (`-Only halogen -Sync -Rebuild`).
- **Scrub-by-key bug (Dean spotted via stale folder dates: Halogen/RMAP_Core/SneezeSDK).** SneezeSDK *did* rebuild (headers fresh); its old folder mtime is cosmetic (header-only nested `install/include` copy). Underlying bug: the install-tree scrub used `libs/<key>` not `libs/<Folder>`, a no-op for the ~13 deps whose folder≠key. Fixed `Remove-DepStateAt` + `Remove-DepState` (PS, via `Get-DepPin`) and `remove_dep_state` + inline `invoke_sync` scrub (bash, new `dep_folder` helper). RMAP_Core = confirmed leftover (Dean purged it).
- **README overhaul** (reconciled against `deps/dependencies.json` + scripts + tree): dependency table rewritten with all 23 deps and tag/branch/commit-tagged refs (was missing freetype/fastgltf/sneeze-sdk/map; Filament/Halogen/BoringSSL refs stale); added `-Verify` + corrected `-Sync` to the flags table + a new "Keeping dependencies in sync" section; Python marked required-by-scripts; "Verifying the Build" rewritten for the single `SneezeTest` exe + suite flags; `src/` tree corrected (`sneeze/context/deps/persona`); Documentation section updated (curated `docs/` wiki moved out to `MetaversalCorp/SneezeDoc`); layout + "adding a dependency" steps updated for the manifest model.
- All scripts re-validated (PS parse, `bash -n`, UTF-8/encoding byte-checks). **Not committed** — Dean commits himself after walking Samantha. The pending working tree is the **entire dependency-management overhaul** (new: `deps/dependencies.json`, `DepGraph.cmake`, `DepVerify.cmake`, `verify.cmake`, `Dependencies.md`, `tools/DepGraph/`, `scripts/ci-dep-fingerprint.sh`, `ci-check-tiers.py`; modified: all `deps/*.cmake` recipes, `deps/CMakeLists.txt`, `src/CMakeLists.txt`, all four `scripts/build-*`, `scripts/dep-sync.ps1`, `.github/workflows/build-platform.yml`) **plus** this session's two fixes and the `README.md` / `project.mdc` / `session` doc updates.

## 2026-08-05 (Wed) ~evening PT — Dean Abramson

**WASM ABI overhaul — HOST root handle, FABRIC node-tree subsystem, SERVICES subsystem, host payload-size validation. (Full detail in `project.mdc` "WASM ABI Overhaul" section.)**

- **ABI (`sneeze_abi.h`):** added `FABRIC` (type 7) node-tree subsystem — `NODE_MAP_SERVICE`, `NODE_MAP_SERVICE_EX`, `NODE_MAP_DATA`, `NODE_ROOT`, `NODE_OPEN`, `NODE_CLOSE` (order deliberate: browser-assigned mapping first, guest-assigned last; map-service is mutually exclusive with manual node edits). Added `SERVICES` (type 12, symmetric with DATA — HAS/GET by service name, returns whole JSON as text). Marked the four `SCENE_NODE_*` methods DEPRECATED (SCENE itself stays; its ambient/directional/background methods staged for later). Documented `NODE_ROTATION`. New 592-byte `SNEEZE_ABI_MAP_SERVICE` wire struct.
- **Staged rollout (Dean's plan):** (1) copy SCENE node methods to FABRIC + add map-service, deprecate old — DONE; (2) deploy new Artemis; (3) migrate fabrics; (4) remove `SCENE_NODE_*` and implement SCENE's light/background methods from 1.
- **Guest SDKs (C/Cpp/Rust):** renamed root handle FABRIC → **HOST** (`pHost`), persistent per-fabric, allocated at Open / freed after Close, per-fabric registry (`Host_Acquire/Find/Release`). C = `typedef struct { uint64_t twFabricIx; } HOST;`; C++ `pHost->Fabric().Node_Root(...)`; Rust `pHost.Fabric().Node_Root(...)`. Dropped the SCENE node wrappers from all SDKs (empty SCENE scaffold left in C++/Rust for the future methods).
- **Host dispatch hardening (`HostFunctions.cpp`):** new bounds-checked `PAYLOAD` cursor + `Exact()` gate on every dispatcher (exact-size conformance, no over/under-read); `size_t` overflow fix on `nPtr+nLen`; `memcpy` unaligned MAPOBJECT fix; `dwSize==0` header-alias fix; proper UTF-8→UTF-16 name conversion via shared `SNEEZE::Name_Set` (reused by `RmcObject.cpp`); inspector System Console `Warn` on each deprecated SCENE call.
- **MSF:** removed the fixed `MSF::SERVICE` struct → generic `nlohmann::json` services (`Service_Add/Remove/Has/Service/Service_Names`); `MSF::Payload()` now returns `const&` (no deep copy). Include hygiene: dropped redundant `<string>` (Map_Object.h) + `<cstring>` (Map_Object.cpp).
- **Consumers:** migrated `map.wasm` (`tools/wasm/map`), `SignMsf`, `convert_dfw.py`, `JwsTest.cpp`. Migrated + recompiled + re-signed the **ArtemisLogo** module (not a git repo); new hash `sha256-f09ff7ae...`; runs live. Fixed two Artemis build breaks (`Input_Key` 3→8 args in Canvas — Dave's, already committed; `MSF::SERVICE` removal in `Network_Detail_Preview.cpp` + `MsfCli.cpp` — uncommitted, this session).
- **Live issue (not this work):** fabric camera pose overwritten by Dave's WASD camera code — Dave to fix.
- **Committed all six repos** (C → Cpp → Rust → SneezeSDK → Sneeze → Artemis) per Dean's explicit instruction; push left to Dean. ArtemisLogo not versioned. Confirmed for Dean that his "3 SDK sub-projects + 1 master + Sneeze + Artemis" tally is right, and that Artemis also carried `MsfCli.cpp`.

## 2026-09-16/17 (Wed/Thu) - Dean Abramson

**WASM NETWORK subsystem (REQUEST + SOCKET) finished across engine and all five SDKs, then the whole release prep. (Full detail in `project.mdc` "WASM NETWORK Subsystem" section.)**

- **WebSocket implementation completed** host-side (`Socket.cpp`: `SOCKET`, `SOCKET_HUB` on one asio io thread, `ISOCKET`/`ISOCKET_LINK`, BoringSSL TLS, 16 MB frame cap) plus the guest service (`Wasm_Network.cpp`) and a new signal-driven **NETWORK agent pool** (`AgentNetwork.cpp`, `kPOOL_NETWORK` = 4, 4 agents at ~1000 Hz) so nothing enters wasmtime from a FETCH agent or the io thread.
- **AS and C# SDKs brought fully current** (they had been left behind): HOST rename, SERVICES, FABRIC node subsystem, C#'s missing Chrono/Moment/Performance/Timer, then REQUEST + SOCKET in both. All five SDKs now expose `pHost->Network()` and the five `INSTANCE` hooks.
- **Rubidium build breaks, both root-caused and fixed twice (CMake + hand-maintained vcxproj):** (1) `boost/version.hpp` - websocketpp/asio were inheriting `_WEBSOCKETPP_CPP11_STL_` and `ASIO_STANDALONE` transitively from RMAP; now declared explicitly in both build systems. No patched websocketpp needed. (2) unresolved `g_szCaCertPem` - BoringSSL has no OS trust store, so the CA bundle is mandatory; the inline CMake generator became `tools/GenCaCert/gencacert.py`, called by CMake and by a new `GenCaCert` MSVC pre-build target.
- **PCH hardening:** `WIN32_LEAN_AND_MEAN` before `<windows.h>` (winsock's `SOCKET` and wincrypt's macros collide with asio/websocketpp/BoringSSL), explicit `<mmsystem.h>`, five more `wincrypt.h` undefs.
- **Deleted `vcpkg.json`** and unwound `USE_LOCAL_DEPS` from `src/CMakeLists.txt` - the non-local path could never have satisfied a build. Dean: "I think that file was created ages ago ... never ended up going down that road."
- **Two bugs found by the live `testapi.wasm` run:** `bClean` was hardcoded true (now derived from websocketpp's remote close code, matching `CloseEvent.wasClean`), and Postman's echo drops binary frames (test moved to `wss://echo.websocket.org`, matching on content instead of frame count). Added `NetworkTest` Test 34 for close reporting; suite now 34 tests.
- **Explained three things to Dave:** why asio/websocketpp were in `vcpkg.json` (they weren't needed there - file deleted), why `cacert.pem` is required now that sockets use BoringSSL directly (curl on Windows had been hiding it behind Schannel), and why `msvc/Sneeze.vcxproj` is still hand-maintained (verified from the tree - it was never restored to autogenerated).
- **Release prep:** all five SDKs + superproject to **0.2.0** (VERSION, Cargo.toml, package.json, in-source fallbacks, README examples); `testapi` repointed at the published crate `sneeze = "0.2.0"` with `Cargo.lock` restored (Dave publishes); `NOTICE` reconciled (asio, WebSocket++, socket.io-client-cpp, Mozilla `cacert.pem` under MPL 2.0, libwebp, RMAP, Map); dependency tables in `README.md` + `project.mdc` reconciled against `deps/dependencies.json` (asio/websocketpp now direct deps, libwebp added, Filament/Halogen/BoringSSL refs corrected); orphaned `src/cmake/cacert_data.cpp.in` deleted; `Control.md` / `Network.md` / `Wasm.md` reconciled.
- **Checked and reported:** no UTF-16 files remain anywhere in the tree or the SDK submodules; Rubidium's tree is clean so it needs no commit; the three new sources are wired into both build systems. Open risks recorded: the new sources have only been compiled with MSVC, the new tests hit live endpoints, and socket.io-client-cpp defaults to `verify_none` for TLS.
- Cursor's UTF-16 write bug hit once (the `project.mdc` append file); caught by the byte check and re-encoded. SneezeDoc deferred at Dean's direction.
